...
--Making tables and inserting values.
...
INSERT INTO user_reactions(reaction_id, user_id, post_id, reaction_type, reaction_date)
...
FROM
    posts p
LEFT JOIN
    user_reactions ur ON p.post_id = ur.post_id
WHERE
...
AVG(COUNT(*)) OVER (PARTITION BY DATE(ur.reaction_date)) AS avereage_reaction_per_user
FROM 
user_reactions ur
WHERE
...
CASE WHEN ur.reaction_type ="comment" THEN 1 ELSE 0 END) AS total_reaction
FROM 
posts p
LEFT JOIN
user_reactions ur ON ur.post_id = p.post_id
WHERE 
